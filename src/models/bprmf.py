...
class BprmfPredictor(BasePredictor):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for _ in range(self.n_loop_cf):
                users, pos_items, neg_items = generate_batch_cf(
                    self.dataset.dict_train_cf, self.batch_size, self.dataset.n_items)
            ...
            if (epoch % self.interval_evaluate) == 0:
                ...
                hits, precisions, recalls, ndcgs, _ = self._evaluate_batch(
                    dict_train_cf=self.dataset.dict_train_cf,
                    dict_test_cf=self.dataset.dict_test_cf,
    ...
    def _evaluate_batch(
        self, dict_train_cf: UserItems, dict_test_cf: UserItems, users_batch: List[th.Tensor], items: th.Tensor, list_k: List[int], is_predict: bool = False
        ...
        with th.no_grad():
            for users in users_batch:
                ...
                hits_batch, precision_batch, recall_batch, ndcg_batch, rec_items_batch = compute_metrics(
                    cf_scores_batch.cpu(), dict_train_cf, dict_test_cf,
